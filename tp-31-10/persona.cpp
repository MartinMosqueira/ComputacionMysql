...
{
	sql::Resultset* personas = myconnection.query("SELECT persona.dni FROM persona WHERE persona.dni == getDni(x)");
	sql::Resultset* personas = myconnection.query("UPDATE persona.nombre, persona.apellido, persona.dni FROM persona");
    long  x;
    string apellido, nombre;
    cout << "Ingrese el dni: ";
    cin >> x;
